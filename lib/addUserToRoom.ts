...
export async function joinUserToRoom(
    ...
    if (isUserInRoom) {
        console.log(`User ${user.username} is already in the room ${room.displayName}`);
        return;
    }
    ...
    try {
        ...
        const roomUpdater = (await modify.getUpdater().room(room.id, user)).addMemberToBeAddedByUsername(user.username);

        // Finalize the update
        await modify.getUpdater().finish(roomUpdater);

        console.log(`User ${user.username} has been added to the room ${room.displayName}`);
    } catch (error) {
        console.error(`Failed to add user ${user.username} to the room ${room.displayName}:`, error);
    }
}